...
class QWenAttention(nn.Module):
    ...
    def _attn(self, query, key, value, causal_mask=None, attention_mask=None, head_mask=None):
        ...
        #print(attn_weights[0,0,-13,:])
        #import pdb;pdb.set_trace()
        if attention_mask is not None:
            #print(attn_weights.shape,attention_mask[:,0,:,:].shape)
            attn_weights = attn_weights + attention_mask
            #attn_weights_0 = attn_weights + attention_mask[:,0,:,:].unsqueeze(1)
            #attn_weights_1 = attn_weights + attention_mask[:,1,:,:].unsqueeze(1)
        #print(attn_weights[0,0,-13,:])
        if self.softmax_in_fp32:
            attn_weights = nn.functional.softmax(attn_weights.float(), dim=-1)
        else:
            attn_weights  = nn.functional.softmax(attn_weights, dim=-1)
            #attn_weights_0 = nn.functional.softmax(attn_weights_0, dim=-1)
            #attn_weights_1 = nn.functional.softmax(attn_weights_1, dim=-1)
            #attn_weights = (attn_weights_0 * 0.7 + attn_weights_1 * 0.3)
        #print(attn_weights[0,0,-13,:])
    ...
    def forward(
        ...
        position_ids: Optional[List[List[torch.Tensor]]] = None,
        ...
        if rotary_pos_emb_list is not None:
            ...
            if len(rotary_pos_emb_list) == 1:
                ...
                rotary_pos_emb = [i.squeeze(2).squeeze(0)[position_ids.to(torch.long)] for i in rotary_pos_emb]
                rotary_pos_emb = (rotary_pos_emb,) * 2
                q_pos_emb, k_pos_emb = rotary_pos_emb
                q_pos_emb = [i.unsqueeze(2) for i in q_pos_emb]
                k_pos_emb = [i.unsqueeze(2) for i in k_pos_emb]
        ...
        if (
            ...
            if not self.use_cache_quantization and SUPPORT_TORCH2:
                if attention_mask is not None:
                    if attention_mask.size() != (bsz, 1, q_len, key_size):
                        raise ValueError(
