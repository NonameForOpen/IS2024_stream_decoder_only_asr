...
def add_sos_eos(ys_pad: torch.Tensor, sos: int, eos: int,
    ...
    ys_in = [torch.cat([_sos, y], dim=0) if y.size(0) != 0 else _sos for y in ys]
    ys_out = [torch.cat([y, _eos], dim=0) if y.size(0) != 0 else _eos for y in ys]
...
def get_activation(act):
    ...
    from wenet.transformer.swish import SquaredReLU
    ...
    activation_funcs = {
        ...
        "squaredrelu": getattr(torch.nn, "SReLU", SquaredReLU),
        "gelu": torch.nn.GELU
    }
