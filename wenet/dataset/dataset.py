...
def Dataset(data_type,
    ...
    if len(id2sy) != 0:
        dataset = Processor(dataset, processor.convert_to_sy, id2sy)
    #if llm_type == 'baichuan':
    #    dataset = Processor(dataset, processor.tokenize_baichuan, bpe_model)
    #elif llm_type == 'cllama2':
    #    dataset = Processor(dataset, processor.tokenize_cllama2, bpe_model)
    #else:
    dataset = Processor(dataset, processor.tokenize, symbol_table, bpe_model,
                        non_lang_syms, conf.get('split_with_space', True))
    token_mask = conf.get('token_mask', False)
    token_mask_conf = conf.get('token_mask_conf',{})
    if token_mask:
        dataset = Processor(dataset, processor.token_mask, symbol_table, **token_mask_conf)
    ...
    #volume_perturb = conf.get('volume_perturb', False)
    #if volume_perturb:
    #    dataset = Processor(dataset, processor.volume_perturb)

    #pitch_shift = conf.get('pitch_shift', False)
    #if pitch_shift:
    #    dataset = Processor(dataset, processor.pitch_shift)
